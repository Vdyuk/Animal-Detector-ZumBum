...
class EfficientNet_V2(nn.Module):
    def __init__(self, n_out):
        ...
        # Define model
        self.effnet =  timm.create_model( 'tf_efficientnetv2_l.in21k_ft_in1k', pretrained=True, num_classes=n_out) # remove classifier nn.Linear )
...
def create_dataloader(images, targets, trn_idx, val_idx):
    weights = [1 / targets.count(1), 1 / targets.count(0)]
    ...
    # Data Loader
    train_loader = DataLoader(train_datasets, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, sampler=sampler)
    valid_loader = DataLoader(valid_datasets, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, sampler=sampler)
...
if __name__ == "__main__":
    ...
    crops, target = all_crops("data/images/",
                              "data/annotation.csv")  # all_crops("D:\\hacks\\train_data_minprirodi\\images\\", "D:\\hacks\\train_data_minprirodi\\annotation.csv")
    print(len(crops))
    SEED = 42
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        torch.save(model.state_dict(), f"weights_class{fold}.pth")
